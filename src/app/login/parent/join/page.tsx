...
export default function ParentJoinPage() {
  ...
  async function verifyCode() {
    ...
    try {
      ...
      if (data.targetRole !== "PARENT") {
        setError("학부모용 초대코드가 아닙니다.");
        return;
      }
      setVerifiedChild(data.child?.name ?? "원아");
    } catch {
  ...
  return (
    <div className="space-y-5 pt-2 pb-6">
      <PassbookShell open={false} tagline="초대코드로 학부모 통장 연결" />
        ...
        <div>
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="예) DK-CHILD-000006"
              className="input-warm flex-1 px-3 py-2.5 text-sm"
            />
            <button
              type="button"
