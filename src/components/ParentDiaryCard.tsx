...
export function ParentDiaryCard({
  ...
  const { state, completeDiaryDeposit } = useApp();
  const [toast, setToast] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [justSaved, setJustSaved] = useState(false);

  const deposited = isDiaryDepositDone(childId, report.date, state.diaryDeposits);
  const balance = getChildTotalSaved(childId, state.passbookTransactions);
  ...
  async function handleTap() {
    ...
    setLoading(true);
    const { alreadyDone, error } = await completeDiaryDeposit(childId, report.date);
    setLoading(false);
